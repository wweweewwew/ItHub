...
function createCatCard(cat) {
  ...
  age.innerHTML = `<strong>Возраст:</strong> ${cat.age} лет`;

  const rating = document.createElement("p");
  rating.innerHTML = `<strong>Рейтинг:</strong> <span class="rating">${"★".repeat(
    Math.round(cat.rate / 2)
  )}${"☆".repeat(5 - Math.round(cat.rate / 2))}</span>`;
  ...
  header.prepend(name);
  details.append(age, rating);
  info.append(header, details, description);
